...
def validate_claimitems(claim, save=True):
    ...
    pricelist_details = ItemsPricelistDetail.objects.filter(
    ...
    pricelist_dict = {pd.item_id: pd for pd in pricelist_details}
...
def validate_claimservices(claim, save=True):
    ...
    pricelist_details = ServicesPricelistDetail.objects.filter(
        service_id__in=service_ids,
        services_pricelist=claim.health_facility.services_pricelist,
        services_pricelist__validity_to__isnull=True,
        *filter_validity(validity=target_date)
    )
    pricelist_dict = {pd.service_id: pd for pd in pricelist_details}
...
def validate_claimitem_in_price_list(claim, claimitem, pricelist_dict=None):
    ...
    if pricelist_dict is None:
        ...
        if not pricelist_detail_qs.exists():
...
def validate_item_product_family(claimitem, target_date, item, insuree_id, adult):
    errors = []
    found = False
    products_data = get_products(target_date, item.id, insuree_id, adult, 'Item')
    for (product_id, product_item_id, insuree_policy_effective_date, policy_effective_date, expiry_date,
         policy_stage) in products_data:
        found = True
...
def validate_service_product_family(claimservice, target_date, service, insuree_id, adult, base_category, claim):
    errors = []
    found = False
    products_data = get_products(target_date, service.id, insuree_id, adult, 'Service')
...
def get_products(target_date, elt_id, insuree_id, adult, item_or_service):
    ...
    qs = model.objects.filter(
        ...
        **{f'{field}_id': elt_id},
        ...
        'product_id', 'prod_elt_id', 'insuree_policy_effective_date', 'policy_effective_date',
        'expiry_date', 'policy_stage', 'waiting_period'
    )
    data = list(qs)
    data.sort(key=lambda row: row[3] + datetimedelta(months=row[6]))
